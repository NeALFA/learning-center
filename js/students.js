...
const teacher = url.get("teacher");
const studentsCard = document.querySelector(".students-card_container");
console.log(teacher);
...
async function getStudentsData() {
  console.log("Error");
  studentsCard.innerHTML = "Loading...";
  const res = await request.get(`teacher/${teacher}/students`);
  console.log(res.data);
  studentsCard.innerHTML = "";
  res.data.map((teacher) => {
    studentsCard.innerHTML += getStudentsCard(teacher);
  });
}
getStudentsData();
function getStudentsCard(student) {
  return `
          <div class="card">
          <div class="card__title">
            ...
            <div class="card-title">
              <h1 class="firstName">${student.firstName}</h1>
              <span><h1 class="lastName">${student.lastName}</h1></span>
              <span><p>| ID: ${student.id}</p></span>
            </div>
          </div>
          <div class="card_descripion">
            ...
            <div class="groups">
              <a href="tel:+998996492639">Phone:${student.phoneNumber}</a>
              <p>isMarried: ${student.isMarried ? "Married" : "Single"}</p>
            </div>
            <div class="buttons">
              <button class="btn edit-btn" onclick="openModal(${
                student.id
              })">Edit</button>
              <button class="btn delet-btn" onclick="deleteStudentData(${
                student.id
              })">Delet</button> 
...
async function deleteStudentData(studentid) {
  await request.delete(`teacher/${teacher}/students/${studentid}`);
  getStudentsData();
}
...
form.addEventListener("submit", async function (e) {
  ...
  if (selected === null) {
    await request.post(`teacher/${teacher}/students`, student);
  } else {
    await request.put(`teacher/${teacher}/students/${selected}`, student);
  }
  this.reset();
  getStudentsData();
});
...
async function openModal(studentid) {
  modal.style.top = 0;
  let { data } = await request.get(`teacher/${teacher}/students/${studentid}`);
  selected = studentid;
  console.log(data);
  modalBtn.textContent = "Edit student date";
...
addBtn.addEventListener("click", () => {
  modal.style.top = 0;
  selected = null;
  modalBtn.textContent = "Add student";
});
